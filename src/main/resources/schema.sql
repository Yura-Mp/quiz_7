...
CREATE TABLE IF NOT EXISTS userAccount (
  userId VARCHAR PRIMARY KEY, -- ユーザID(主キー)．
  userName VARCHAR NOT NULL, -- ユーザの名前．サービス上で表示される名前として用いられることを想定．
  pass VARCHAR, -- アカウントのパスワード．データとしてはbcryptでハッシュ化されたデータを保持することを想定．NULL(パスワード未設定)である場合，パスワードなしでログインできるものとする．
